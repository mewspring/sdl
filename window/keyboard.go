...
func getMod() (mod we.Mod) {
	return weMod(C.SDL_GetModState())
}

// weMod returns the corresponding we.Mod for the provided SDL_Keymod.
func weMod(m C.SDL_Keymod) (mod we.Mod) {
...
// weKey returns the corresponding we.Key for the provided SDL_Keycode.
func weKey(keycode C.SDL_Keycode) (key we.Key) {
	switch keycode {
	// Printable keys.
	case C.SDLK_SPACE:
		return we.KeySpace
